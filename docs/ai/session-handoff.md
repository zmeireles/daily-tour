...
> 2. **Daily Tour** is the next desktop screen but has **3 backend deps (all root-caused, fixes ready)** — do these FIRST, then build the screen:
>    - **(a) BUG1 — itinerary copy renders ENGLISH on a PT-PT app.** Root cause: JWT carries a `locale` claim but the BFF drops it → planner defaults to `en` → the LLM faithfully writes English. **Fix (small, cross-service):** `services/bff/src/routes/tour-plans.ts:25` read `locale` from `req.user` + pass to `createTourPlan`; `services/bff/src/lib/planner-client.ts` add `locale` to params+body; `services/planner-svc/.../workers/plan_worker.py` `_build_plan_request` accept `locale` → `PlanRequest(locale=...)`; ALSO `tour-plan-view.ts:51` `pickName()` take a locale arg + prefer `names[locale]` before `en` (1b — English on the 28 bilingual landmarks). Update BFF tests (`tour-plans.test.ts`, `tour-plan-view.test.ts:160`) + a planner assembler locale assertion. Deploys BFF + planner. Verify: regenerate a tour on qual, copy is PT-PT.
>    - **(b) Stop coordinates for the Stage-2 route map.** `TourStop` (`timeline-stop.tsx:4`) has NO lat/lng; the planner HAS `geom_lat/lng` and the BFF `tour-plan-view.ts toStop`/`resolvePlaceMeta` just don't thread it. **Fix (BFF):** resolve `geom` in `PlaceMeta` + include in `TourStop` + shared-types. Then `TourRouteMap` can plot numbered markers + polyline.
>    - **(c) ✅ BUG2 DONE (relevance).** The 4 guesthouses were mis-tagged with a `see` `place_action_wish` row in qual → planner correctly returned them. Deleted the 4 tags directly on qual → guesthouses now have ZERO action tags (invisible to discover + tours = correct for lodging). They're NOT in any committed seed (admin/listings-inserted), so nothing to re-seed; if re-created via admin, do not add action tags.
> 3. **Build Daily Tour desktop** (`wf2-design.md` §2.4, `engageAt="md"`): `DailyTourDesktop` (2 stages) — Stage 1 INTAKE = `TwoPaneIntake` (form card + `EditorialImageryPanel`, both contained); Stage 2 TIMELINE = rail frame: `TourItineraryRail` (`DaySummaryRail` + unforked `DailyTourTimeline` in a `data-density="desktop"` wrapper + pinned Partilhar/Voltar toolbar) + `TourRouteMap` (numbered sibling-badge markers on the unforked `MapView` + tea-green polyline — needs dep (b)). `StopSyncBridge` (outer event-delegation) for rail↔map sync (timeline exposes no hover cb). Wrap `_authed.tour.new.tsx` + `_authed.tour.$planId.tsx` in `ResponsiveScreen`. Then **Place Detail** (§2.3) → **Chat** (§2.5).
> 4. Optional: run the 5 Stitch prompts (web UI) to mock screens.
>
> ### Deferred / notes
>
> - **Home cover-story** — DONE (#274, open PR): first pick spans 2×2 when ≥3 picks (magazine focal point + balances the right-rail rhythm gap). No PlaceCard fork.
> - Discover **stray map arc** — `fitToPins` (#272) killed the bold arc (0% magenta in UAT); a FAINT pale-lilac fit-bounds remnant remains. NO line-layer in code (`buildStyle` = OSM raster; only purple is the `MapPin` `--hydrangea-400` selection ring). True source needs a live maplibre-instance inspection (map not globally exposed) — low ROI, deferred.
> - The spec's "Overline contrast hardening" = **no-op** (token already 6.8:1 AA on cream) — skipped, verified.
> - **State:** main `8f8f315` (#267–#273 all merged + LIVE on qual); **1 open code PR: #274** (Home cover-story). qual data fixes applied directly this session (not in seed): picks re-point (#271-companion), guesthouse action-tag delete (BUG2). dt-tests `review` empty. Desktop Home (≥768) + Discover (≥1024) live at `https://qual.stay.portugalodyssey.pt`.
