...
>
> - **T-2.B.0 — Stitch mockups: PARTIAL (Home done; 4 prompts ready).** The Stitch project `Daily Tour — São Miguel PWA` (`projects/11661203433672958283`) already had a polished **"São Miguel Editorial"** design system (asset `8a6674ad896243c8881fc985aee6f504`, dark Field-Journal, our palette) + a generated **Home** screen (2 iterations). Captured the design system into `docs/design/DESIGN.md` (was Stitch-only) + saved Home full HTML to `docs/design/stitch/home-v{1,2}.html`. **BLOCKER:** the MCP `generate_screen_from_text` times out at the transport layer and the generation does NOT persist (4 attempts, ~20 min; reads work, only generation fails — transport cancels the long op). The 4 remaining prompts (Place Detail, Discover, Daily Tour, Chat) are saved in `docs/design/stitch/prompts.md` → **run them in the Stitch web UI** (labs.google.com/stitch; project + design system already exist, no MCP timeout), then download to `docs/design/stitch/`. **Reconcile note:** Stitch DS uses Newsreader where the app uses Fraunces — standardise on Fraunces at implementation.
...
> - **Remaining Slice 2.B:** finish T-2.B.0 (4 screens via web UI), **T-2.B.1** brand mark + icon regen (needs human steer), **T-2.B.3** photography for 28 places (`temp/place-photo-sourcing.md`; commission vs Commons-curated decision).
> - **State (uncommitted, pending PR decision):** modified `pt-PT/admin.json` + `place.json` + this handoff; new `apps/pwa/src/locales/README.md`, `docs/design/DESIGN.md`, `docs/design/stitch/*`. `main` otherwise synced; **0 open PRs**, no cs-agent worktrees, dt-tests `review` empty. Plan-006 features live on qual (run 27548580776). The dev-stack note below is obsolete (stack stays down; qual is the live target).
