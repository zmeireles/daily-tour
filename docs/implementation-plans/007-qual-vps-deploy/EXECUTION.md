...
**Both fix PRs (ESCALATE) were merged on the human's go**, serialized via `update-branch` → CI → squash. `publish-images` rebuilt `bff:qual`+`pwa:qual` @ f3d216e (fix baked in), then `deploy-qa.yml -f image_tag=qual` redeployed — **all steps green**, including the new "Provision owner access (akadmin → staff)" step and the smoke now exercising `/v1/r/`. **Guest cold-entry re-UAT'd PASS** in a real browser: cold nav to `/r/<token>` → `200 text/html`, SPA boots, XHR `/v1/r/` `200 {jwt}`, lands authed home, `/v1/discover` `200` under the guest Bearer JWT (`temp/uat-plan007/G1-spa-boot.png`, `G3-authed-home.png`). **→ Plan-002 2.A guest-journey exit criterion MET.**

Then **#238** seeded the "Casa do Sol" guesthouse into the catalog `dev.ts` seeder (`#152`, idempotent, runs in deploy stage 5) — merged, redeployed (stage 5 inserted the row on the live DB, no wipe/hand-patch), and the **owner-edit re-UAT'd PASS**: `/admin/places` renders a Hide/Show control on every row (no bare "—"); toggling "A Tasca (PDL)" fired PUT 200 (`hidden_place_ids {…021}`) then DELETE 200 (`{}`), data left as found (`temp/uat-plan007/R1–R4*.png`). **Plan-007 is CLOSED** — every criterion verified live (deploy reproducibility · guest journey · owner login · owner edit · hero/credit). Riff #157 + #152 done. Non-blocking follow-ups remain: #158 (registerSW.js 404/MIME), osrm re-enable, and a deeper owner create+photo+publish qual UAT.
