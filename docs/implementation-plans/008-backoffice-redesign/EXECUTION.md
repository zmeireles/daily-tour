...
## Next — Slice 3 (fresh session)

Only **Slice 3** (form excellence + Helper A translate, `T-8.3.x`, beta-desirable) remains — deferred to a fresh session by the owner. It needs a **new `POST /v1/admin/translate` BFF endpoint** (Claude via `ANTHROPIC_API_KEY`, European-Portuguese pré-AO, do-not-translate list, owner-auth + rate-limited) + the **`es` data-model addition** (T-8.3.4 step 0: add `es` to the place/guesthouse/profile form schemas + body builders from one config array) + `TranslatableField`/`useFieldTranslation` (§7a). Slice 5 (map picker + polish, post-beta) is the last after that.
