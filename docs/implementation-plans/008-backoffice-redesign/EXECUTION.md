...
- **Owner-feature UAT owed**: the redesigned forms + translate helper are Fable-gated + unit-tested but not walked live on qual (needs the **akadmin** Authentik password — human). Slice 3 is **not yet deployed to qual** (owner-only; user's call).

## Next — Slice 5 (post-beta)

Only **Slice 5** (Helper B map picker + polish, `T-8.5.x`, post-beta) remains: the geocoder decision (Photon vs commercial, T-8.5.1) + BFF `POST /v1/admin/geocode` proxy, the `LocationPicker` (replaces the numeric lat/lng the forms still use), the beta-metrics real dashboard, and calm motion polish. Plan-008 is otherwise code-complete.
