...
Status: **CODE-COMPLETE** — **all 6 slices (0–5) DONE**, every merge Fable-gated. Slice 5 (map picker + polish) landed 2026-07-11, main `4f62245` (PRs #355–#358). **Deploy state**: Slices 0–2 + 4 on qual (main `12cc6d5`); **Slice 3 deployed to qual 2026-07-09** (main `4c2c4d7`, browser-UAT 9/9, guest app not regressed); **Slice 5 NOT yet deployed** (owner-only — user's call; `image_tag` = full SHA of `4f62245`; needs `GEOAPIFY_API_KEY` in `.env.qual` for a live map picker). **Owner-feature UAT** of Slices 3–5 still owed (akadmin password). Full detail + acceptance criteria in [`EXECUTION.md`](./EXECUTION.md) + [`README.md`](./README.md). Design source of truth: [`docs/design/backoffice-redesign/proposal-001.md`](../../design/backoffice-redesign/proposal-001.md). Task IDs `T-8.<phase>.<task>` (middle digit = proposal phase 0–5).
