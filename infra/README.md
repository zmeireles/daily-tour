...
| [`compose/docker-compose.app.yml`](./compose/docker-compose.app.yml)   | bff + pwa-static behind Traefik (T-0.4.3)                                        |
...
pnpm --filter @daily-tour/pwa build   # populate apps/pwa/dist for the nginx bind mount
docker compose --env-file dev-environment \
  -f infra/compose/docker-compose.base.yml \
  -f infra/compose/docker-compose.traefik.yml \
...
`--build` is required because the BFF service has a `build:` block — drop the flag and Compose will reuse a stale image (or fail if it has none). For hot-reload feature work, use the **native dev flow** instead (see "Dev flows" below) — drop `docker-compose.app.yml` from the chain and run `pnpm dev` directly.
...
```bash
docker compose --env-file dev-environment -f ... ps  # wait until all 11 services healthy
```
...
**Native dev (hot-reload, recommended for feature work)** — Vite serves the PWA with HMR; tsx watches the BFF. Compose still runs the infra (PG, Redis, Rabbit, MinIO, Traefik, Authentik, n8n) — drop the app overlay from the chain.

```bash
# In one terminal
pnpm --filter @daily-tour/pwa dev      # → http://localhost:5173 (NOT through Traefik)
# In another
pnpm --filter @daily-tour/bff dev      # → http://localhost:8080/health (NOT through Traefik)
# Compose runs the rest (no app overlay)
docker compose --env-file dev-environment \
  -f infra/compose/docker-compose.base.yml \
  -f infra/compose/docker-compose.traefik.yml \
  -f infra/compose/docker-compose.authentik.yml \
...
```bash
pnpm build                              # build PWA + BFF artifacts
docker compose --env-file dev-environment \
  -f infra/compose/docker-compose.base.yml \
  -f infra/compose/docker-compose.traefik.yml \
  -f infra/compose/docker-compose.authentik.yml \
