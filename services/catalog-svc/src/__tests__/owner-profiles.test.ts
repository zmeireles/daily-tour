...
describe("POST/GET/PATCH/DELETE /v1/owner-profiles", () => {
  ...
  it("GET + PATCH + 404 paths", async () => {
    await app.inject({ method: "POST", url: "/v1/owner-profiles", payload: VALID_BODY });

    const get = await app.inject({ method: "GET", url: `/v1/owner-profiles/${OWNER_ID}` });
  ...
  it("DELETE hard-removes + idempotent", async () => {
    ...
    const del1 = await app.inject({ method: "DELETE", url: `/v1/owner-profiles/${OWNER_ID}` });
    expect(del1.statusCode).toBe(204);

    const get404 = await app.inject({ method: "GET", url: `/v1/owner-profiles/${OWNER_ID}` });
    expect(get404.statusCode).toBe(404);

    const del2 = await app.inject({ method: "DELETE", url: `/v1/owner-profiles/${OWNER_ID}` });
