...
`planner_svc/main.py` and the shared suite stays green while this service serves every
...
from planner_svc.main import create_app

GUARDED = "/v1/tour-plans/00000000-0000-0000-0000-000000000000"
INTERNAL_HEADERS = {"x-internal-token": os.environ["PLANNER_SVC_INTERNAL_TOKEN"]}
...
class TestGateIsWired:
    ...
    def test_wrong_credential_is_rejected(self) -> None:
        response = _client().get(GUARDED, headers={"x-internal-token": "wrong"})
        assert response.status_code == 401
    ...
    def test_valid_credential_reaches_the_route(self) -> None:
        ...
        response = _client().get(GUARDED, headers=INTERNAL_HEADERS)
        assert response.status_code != 401
